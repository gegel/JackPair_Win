...
static unsigned char npp_flag=0;
...
**
*****************************************************************************/
unsigned char npp(Shortword sp_in[], Shortword sp_out[])
{
...
static void		process_frame(Shortword inspeech[], Shortword outspeech[])
{
	...
	if ((comp_data_shift(gamma_max, gamma_max_shift, GAMMAX_THR, 6) < 0) &&
        ...
        if(n_flag) npp_flag=0; else npp_flag=0x80;
	if(npp_vad>VAD_LEVEL) npp_vad--; //restrict counter
	else if(npp_flag) npp_vad++; //each voice frame
