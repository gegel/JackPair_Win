...
#include "scalarmult.h" //x25519
#include "trng.h"
#include "cntr.h"

//some definitions
#define VAD_POS 10 //byte position of VAD decission flag
#define DATA_LEN 10 //number of data bytes for used modem type
#define CORR_INTERVAL 8 //lag distance for correcting counter (max correcting time 1' for 10 ppm) 


//global values
static unsigned char mode=0; //0-IKE, 1 -work mode
static unsigned char nokey=0; //1-unencrypted mode
...
static short last_lag=0; //last lag of modem
static unsigned char last_syn=0; //flag of last modem sync state
...
//constant data
static const unsigned char bitmask[8]={1,2,4,8,16,32,64,128}; //bit mask table for byte
static const unsigned char bitset[16]={0,1,1,2, 1,2,2,3, 1,2,2,3, 2,3,3,4}; //count of bit '1' in nibble

//helpers procedures prototypes
static unsigned int m2i(unsigned char* m, unsigned char p); //3 bytes array to int with polarity
static void i2m(unsigned int uu, unsigned char* m); //int to 3 bytes array
static unsigned int m2u(unsigned char* m); //4 bytes array to unsigned int
static void u2m(unsigned int uu, unsigned char* m); //unsigned int to 4 bytes array

//======================================Helpers=====================================

//portable convert 24 bit integer to 3 bytes unaligned array
static void i2m(unsigned int uu, unsigned char* m)
{
 m[0]=uu&0xFF;
 m[1]=(uu>>8)&0xFF;
 m[2]=(uu>>16)&0xFF;
...
static unsigned int m2i(unsigned char* m, unsigned char p)
{
 unsigned int uu;
...
static void u2m(unsigned int uu, unsigned char* m)
{
 m[0]=uu&0xFF;
 m[1]=(uu>>8)&0xFF;
 m[2]=(uu>>16)&0xFF;
...
static unsigned int m2u(unsigned char* m)
{
 unsigned int uu;
...
//make 10 bytes outgoing packet from voice data (m[10] is VAD flag)
void make_pkt(unsigned char* m)
{
 unsigned int u; //intermediate value
 if(!m[VAD_POS])  //no VAD: this is silency
 {          //make control packet
  ...
  m[9]=crc8(m,9); //byte 10 of control packet is CRC 
 ...
 {  //encrypt voice data
  ...
  sh_crp(m, DATA_LEN); //xor data with gamma
 }
...
{
 unsigned char mm[16]; //temporary array
 ...
 if(!(m[11]&0x40)) //check modem carrier detect 
 {
	 last_syn=0;//clear sync flag
	 return 1; //if no carrier returns data flag
 }
	
 //on start sync possible correct input frames counter
 ///////!!!!!!!!!! check pcm_lag position, fix this code!!!
 pcm_lag=m[13]&0x30; //2 MSB of current modem lag
 pcm_lag=(pcm_lag<<4)+m[14]; //8 LSB of current modem lag
 if(!last_syn) //check this is a first sync after sync lost
 { //compare current modem lag with lag of last sync value and correct input frame counter
  if((last_lag	> (720-CORR_INTERVAL))&&(pcm_lag < CORR_INTERVAL))
  {
   cnt_in--; //lag jumped over uppper frame boundary: skip one frame
   //return 0; //on back correcting return without decrypting for safity reason
  }
  else if((last_lag	< CORR_INTERVAL)&&(pcm_lag > (720-CORR_INTERVAL))) cnt_in++; //lag jumped over lower frame boundary: count one extra frame
  last_syn=1; //set last sync flag 
 }
 last_lag=pcm_lag; //on sync permanently set current modem lag as a last lag
 
 //check is this packet a control or data
 ...
 { 
  //try correct actual counter of incoming packets by expected value	
	...
	if((!mode)||(nokey))  //in IKE and unencrypted mode: can correct to any value in both directin
	{
 ...
 else //this is data packet: decrypt it
 {
  ...
  sh_xof(); //permute to gamma
  sh_crp(m, DATA_LEN); //xor data with gamma
  if(pol) for(r=0;r<DATA_LEN;r++) m[r]^=0xFF; //apply channel polarity
...
unsigned char txkey(unsigned char* m)
{
 ...
 m[VAD_POS]=cnt_out%5; //set not_control flag: transmit one control packet followed 4 data packets	
 if(m[VAD_POS]) //make data packet
 {
  p=our_key+9*(m[VAD_POS]-1); //pointer to output data
  memcpy(m,p,9); //output 9 bytes of our key
  m[9]=crc8(m,9); //last byte is crc of part of key
 }
 return m[VAD_POS];	//returns not_control flag
}
...
void setkey(unsigned int pin)
{
	...
	#define JP_SALT (const char*)"jp1_salt"  //constant application-depends salt for hashing pin code
        #define NOKEY (const char*)"1234" //default key for scrambling
        ...
        if(!pin)
        {
         ...
         mode=1;
         nokey=1;
          nokey=1; //set flag of unencrypted mode
         return;
        }

        nokey=0; //clear flag of unencrypted mode
...
void setrand_pc(unsigned char* trn, int len)
{
 while(!getSid(trn, len)){};
 sh_ini(); //init shake
 sh_upd(trn, len); //absorb TRN
 sh_xof(); //permute
 sh_out(sid, 16); //output SID
}
//----------------------------------------------------------------------------------
 ...
 if(cnt_out&0x800000)
 {
  ...
  if(!i) //expected control packet
  {
   if(sync_flag) //actally this packet was detected as a data, not a control
	 ...
	 if(pol) for(i=0;i<DATA_LEN;i++) p[i]^=0xFF; //revert back expected data for inverse polarity
  } //expected data packet:
  else //this packet is a part of key 
  {
	 memcpy(p, their_key+9*(i-1), 9); //copy expected key data to temporary buffer
         p[9]=crc8(p,9); //add crc byte expected on input
  }
  //------------------------------------------------------------
...
//------------------------------------------------------------ 
  ...
  b_all+=(DATA_LEN*8);  //add number of data bits
...
//----------------------------------------------------------- 
 //output statistic:
 sprintf((char*)logout, "D=%d P=%d.%02d(%d) S=%d.%02d(%d) B=%d.%02d(%d) L=%002d C=%d/%d/%d/%d\r\n",	
 //received packet number sequence
  ...
  rate_tune, //rate tuning value
  lr  //average rate difference

 ); //+=8: value for fine tune recording samplerate 
...
void work_ber(unsigned char* m, short lr, signed char* logout)
{
 ...
 //----------------------------------------------------------- 
 //output statistic:
sprintf((char*)logout, "D=%d %c>%c S=%d.%02d(%d) L=%002d C=%d/%d/%d/%d\r\n",	
 //received packet number sequence
...
{
 //global values
  mode=0; //0-IKE, 1 -work mode
  nokey=0; //flag of unencrypted mode
  pol=0;    //channel polarity
  cnt_out=0; //counter of outgoing packets
  cnt_in=0;  //counter of incoming packets
  dcnt=0;    //delta of actual and expected incoming counters
  last_lag=0; //last lag of modem
  last_syn=0; //flag of last modem sync state

//for statistic log
  c_err=0; //absolute number of bad control packets
  c_bad=0; //rate of bad control packets
  c_all=1; //rate of all packets (must be non_zero for division)

  s_err=0; //absolute number of parity errors

  b_err=0; //absolute number of bit errors
  b_bad=0; //bit error rate
  b_all=1; //counter of data bits (must be non_zero for division)

  //data arrays
  memset(sid,0,sizeof(sid));  //sid for PRNG
  memset(secret,0,sizeof(secret)); //our secret key on IKE, encrypting/decrypring keys on work
  memset(our_key,0,sizeof(our_key)); //our public key on IKE
  memset(their_key,0,sizeof(their_key)); //elligator on start, their public key on IKE
  memset(accumulator,0,sizeof(accumulator));  //entropy collector on start, soft bits of their public key on ike
}
